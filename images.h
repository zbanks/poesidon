#include <stdint.h>

// File: 'images/Poseidon_doge_1.png'
#define POSEIDON_DOGE_1_IMAGE POSEIDON_DOGE_1_WIDTH,POSEIDON_DOGE_1_HEIGHT,(uint8_t*)POSEIDON_DOGE_1_DATA
#define POSEIDON_DOGE_1_WIDTH (130)
#define POSEIDON_DOGE_1_HEIGHT (130)
extern const uint8_t POSEIDON_DOGE_1_DATA[16900];

// File: 'images/circle.png'
#define CIRCLE_IMAGE CIRCLE_WIDTH,CIRCLE_HEIGHT,(uint8_t*)CIRCLE_DATA
#define CIRCLE_WIDTH (40)
#define CIRCLE_HEIGHT (40)
extern const uint8_t CIRCLE_DATA[1600];

// File: 'images/doge_water.png'
#define DOGE_WATER_IMAGE DOGE_WATER_WIDTH,DOGE_WATER_HEIGHT,(uint8_t*)DOGE_WATER_DATA
#define DOGE_WATER_WIDTH (130)
#define DOGE_WATER_HEIGHT (130)
extern const uint8_t DOGE_WATER_DATA[16900];

// File: 'images/dot.png'
#define DOT_IMAGE DOT_WIDTH,DOT_HEIGHT,(uint8_t*)DOT_DATA
#define DOT_WIDTH (40)
#define DOT_HEIGHT (40)
extern const uint8_t DOT_DATA[1600];

// File: 'images/line.png'
#define LINE_IMAGE LINE_WIDTH,LINE_HEIGHT,(uint8_t*)LINE_DATA
#define LINE_WIDTH (40)
#define LINE_HEIGHT (80)
extern const uint8_t LINE_DATA[3200];

// File: 'images/splash.png'
#define SPLASH_IMAGE SPLASH_WIDTH,SPLASH_HEIGHT,(uint8_t*)SPLASH_DATA
#define SPLASH_WIDTH (130)
#define SPLASH_HEIGHT (130)
extern const uint8_t SPLASH_DATA[16900];

// File: 'images/txt_bright_laser.png'
#define TXT_BRIGHT_LASER_IMAGE TXT_BRIGHT_LASER_WIDTH,TXT_BRIGHT_LASER_HEIGHT,(uint8_t*)TXT_BRIGHT_LASER_DATA
#define TXT_BRIGHT_LASER_WIDTH (17)
#define TXT_BRIGHT_LASER_HEIGHT (67)
extern const uint8_t TXT_BRIGHT_LASER_DATA[1139];

// File: 'images/txt_much_length.png'
#define TXT_MUCH_LENGTH_IMAGE TXT_MUCH_LENGTH_WIDTH,TXT_MUCH_LENGTH_HEIGHT,(uint8_t*)TXT_MUCH_LENGTH_DATA
#define TXT_MUCH_LENGTH_WIDTH (17)
#define TXT_MUCH_LENGTH_HEIGHT (65)
extern const uint8_t TXT_MUCH_LENGTH_DATA[1105];

// File: 'images/txt_such_speed.png'
#define TXT_SUCH_SPEED_IMAGE TXT_SUCH_SPEED_WIDTH,TXT_SUCH_SPEED_HEIGHT,(uint8_t*)TXT_SUCH_SPEED_DATA
#define TXT_SUCH_SPEED_WIDTH (17)
#define TXT_SUCH_SPEED_HEIGHT (58)
extern const uint8_t TXT_SUCH_SPEED_DATA[986];

// File: 'images/txt_wow.png'
#define TXT_WOW_IMAGE TXT_WOW_WIDTH,TXT_WOW_HEIGHT,(uint8_t*)TXT_WOW_DATA
#define TXT_WOW_WIDTH (17)
#define TXT_WOW_HEIGHT (22)
extern const uint8_t TXT_WOW_DATA[374];

